...
const filterData = reactive({ ...initialState });

const reset = async () => {
  const queryPath: string = `products/?page=${query.page}&limit=${query.limit}&sortBy=${query.sortBy}`;
  Object.assign(filterData, initialState);
  await props.func(
    { path: queryPath, method: "get", withCredentials: true },
    "products"
  );
};
